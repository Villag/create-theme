<div id="user-{{ID}}" class="modal fade person card-back {{types}}" aria-labelledby="modal-person-label" aria-hidden="true" data-type="{{type}}" tabindex="-1" itemscope itemtype="http://schema.org/Person">
		...
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<header class="n" title="Name">
					<img src="{{{avatar}}}" height="55" width="55" alt="{{first_name}} {{last_name}}" class="thumbnail avatar pull-left" itemprop="image">
					<h3 class="fn" itemprop="name">
						<span class="given-name">{{first_name}}</span>
						<span class="family-name">{{last_name}}</span>
					</h3>
					<div class="primary-job" itemprop="jobTitle">{{primary_jobs}}</div>
				</header><!-- .n -->
			</div><!-- .modal-header -->
				...
				<section class="note">
					...
					<ul>
						...
						<?php if( is_user_logged_in() ) { ?>
							{{#if phone}}
								<li class="tel"><abbr class="value" itemprop="telephone" title="+1{{phone}}">{{phone}}</abbr></li>
							{{/if}}
							{{#if email}}
